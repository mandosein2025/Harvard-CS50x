...
void grayscale(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            ...
            media_cores =
                round((image[i][j].rgbtBlue + image[i][j].rgbtGreen + image[i][j].rgbtRed) / 3.00);
...
void sepia(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            ...
            sepiaRed =
                limit(round(.393 * originalRed + .769 * originalGreen + .189 * originalBlue));
            sepiaGreen =
                limit(round(.349 * originalRed + .686 * originalGreen + .168 * originalBlue));
            sepiaBlue =
                limit(round(.272 * originalRed + .534 * originalGreen + .131 * originalBlue));
